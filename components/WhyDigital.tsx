...
export default function WhyDigital() {
  ...
  return (
    <section id="mengapa-digital" className="bg-krem py-16 lg:py-32 relative overflow-hidden">
      ...
      <div className="absolute top-1/2 right-0 w-96 h-96 rounded-full bg-teal/8 blur-3xl pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div ref={ref} className="text-center mb-10 lg:mb-16">
          ...
          <motion.p
            ...
            className="font-outfit text-base lg:text-lg text-navy/60 max-w-2xl mx-auto leading-relaxed"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-12 lg:mb-20">
          {globalStats.map((stat, i) => (
            <motion.div
              ...
              className="bg-white border border-navy/10 rounded-2xl p-6 transition-colors shadow-sm shadow-navy/10"
            >
              ...
              <div className="mb-1">
                ...
                {stat.suffix && (
                  <span className="font-outfit text-sm text-navy/50 ml-1">{stat.suffix}</span>
                )}
              </div>
              <h3 className="font-syne font-bold text-navy text-sm mb-3 leading-snug">{stat.label}</h3>
              <p className="font-outfit text-xs text-navy/60 leading-relaxed">{stat.desc}</p>
            </motion.div>
          ))}
        </div>
        ...
        <div ref={timelineRef}>
          <motion.h3
            ...
            transition={{ duration: 0.5 }}
            className="font-syne font-bold text-xl sm:text-2xl text-navy mb-7 sm:mb-10 text-center"
          >
          ...
          <div className="relative">
            {/* Timeline line */}
            <div className="absolute left-3 sm:left-4 lg:left-1/2 top-0 bottom-0 w-px bg-navy/10" />
            <motion.div
            ...
            <div className="space-y-8">
              {insights.map((item, i) => (
                ...
                >
                  {/* Timeline dot */}
                  <div className="absolute left-3 sm:left-4 lg:left-1/2 -translate-x-1/2 w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-amber border-2 border-krem mt-5" />

                  <div className={`ml-8 sm:ml-10 lg:ml-0 lg:w-5/12 ${i % 2 === 0 ? "lg:pr-12" : "lg:pl-12"}`}>
                    <div className="bg-white border border-navy/10 rounded-xl p-5 shadow-sm shadow-navy/10">
                      <span className="font-syne font-extrabold text-amber text-xl">{item.year}</span>
                      <p className="font-outfit text-sm text-navy/60 mt-2 leading-relaxed">{item.event}</p>
                    </div>
                  </div>
                </motion.div>
              ))}
        ...
        >
          <p className="font-outfit text-navy/60 text-base mb-6 max-w-xl mx-auto">
            Bisnis kamu sudah jalan bertahun-tahun. Yang kurang cuma satu —
            <strong className="text-navy"> kehadiran digital yang rapi dan bisa dikontrol.</strong>
          </p>
          <motion.button
            onClick={() => {
